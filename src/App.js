...
/* ── Topbar ── */
.topbar{
  background:rgba(var(--bg-rgb,17,16,16),0.85);
  border-bottom:1px solid var(--border);
  padding:14px 28px;
...
.stat-card::after{
  ...
  background:radial-gradient(circle, var(--accent-dim3) 0%, transparent 70%);
...
.filter-tab{
  ...
  border-radius:20px;transition:all 0.12s;
  letter-spacing:0.02em;
}
...
/* Priority badges — text only, no emojis */
.p-badge{
...
.task-edit-input{
  ...
  padding:3px 9px;outline:none;border-radius:4px;
  box-shadow:0 0 0 3px var(--accent-dim);
}
...
.section-filter-line{flex:1;height:1px;background:var(--border);}
`;
};
        ...
        <div className="task-title-row">
          ...
          {task.priority && task.priority !== "none" && (
            <span className={`p-badge p-${task.priority}`}>
              {Icon.dot(priorityColor[task.priority])}
              {task.priority}
            </span>
          )}
        </div>
...
function Dashboard({ token, email, onLogout, theme, setTheme }) {
  ...
  const navItems = [
    { id: "tasks", icon: <Icon.tasks />, label: "My Tasks", count: tasks.length },
    { id: "today", icon: <Icon.today />, label: "Today", count: tasks.filter(t => t.dueDate?.startsWith(todayStr)).length || null },
    { id: "priority", icon: <Icon.priority />, label: "Priority", count: tasks.filter(t => t.priority === "high").length || null },
    { id: "projects", icon: <Icon.projects />, label: "Projects", count: projectStats.length || null },
  ];
      ...
      <main className="main">
        ...
        <div className="content">
          ...
          <div className="search-wrap">
            <span className="search-icon-wrap"><Icon.search /></span>
            ...
            <>
              ...
              {selectedProject && (
                <div style={{ marginBottom: 12, fontSize: 12, color: "var(--ink-muted)", display: "flex", alignItems: "center", gap: 8 }}>
                  Showing tasks in <span style={{ color: "var(--accent)", fontWeight: 700 }}>{selectedProject}</span>
          ...
          <div className="task-section">
            <div className="task-section-head">
              <span className="task-section-title">
                {{ tasks: "All Tasks", today: "Today's Tasks", priority: "Priority Tasks", projects: selectedProject || "All Projects" }[page]}
              </span>
                ...
                ? (
                  <div className="empty-tasks">
                    <div className="empty-icon-wrap">
                      {page === "today" ? <Icon.today /> : page === "priority" ? <Icon.priority /> : <Icon.tasks />}
                    </div>
                    <div className="empty-title">
                      {search ? "No results" : page === "today" ? "Nothing due today" : page === "priority" ? "No priority tasks" : "No tasks"}
                    </div>
